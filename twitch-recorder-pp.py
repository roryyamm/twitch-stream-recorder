...
class TwitchRecorder:
	def __init__(self):
		...
		self.custom = False
		self.custom_command = "echo"
		self.recattempt = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
		self.streamtitle = ""
		# user configuration
	...
	def process_recorded_file(self, recorded_filename, processed_filename):
		...
		if self.custom:
			logging.info("Running custom processing on: %s", processed_filename)
			processed_path = os.path.join(self.root_path, "processed", self.username)
			output = os.path.join(self.root_path, "custom.log")
			fullcomm = self.custom_command + ' ' + processed_path + ' ' + processed_filename + ' ' + self.recattempt
	...
	def loop_check(self, recorded_path, processed_path):
		while True:
			...
			if status == TwitchResponseStatus.NOT_FOUND:
				...
				logging.info("%s: %s currently offline, checking again in %s seconds",self.recattempt, self.username, self.refresh)
				time.sleep(self.refresh)
				...
				self.streamtitle = channel.get("title")
				filename = self.username + " - " + datetime.datetime.now() \
					.strftime("%Y-%m-%d %Hh%Mm%Ss") + " - " + self.streamtitle + ".mp4"
...
def main(argv):
	...
	try:
		opts, args = getopt.getopt(argv, "hu:q:l:", ["username=", "quality=", "log=", "logging=", "disable-ffmpeg", "custom="])
	...
	for opt, arg in opts:
		if opt == "-h":
			...
			twitch_recorder.custom = True
			twitch_recorder.custom_command = arg
			if os.path.isfile(os.path.join(config.root_path, twitch_recorder.custom_command)):
				twitch_recorder.custom_command = os.path.join(config.root_path, twitch_recorder.custom_command)
			logging.info("Custom processing script %s", twitch_recorder.custom_command)
